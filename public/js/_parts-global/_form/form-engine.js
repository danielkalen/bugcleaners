...
var multiStep,
Form = function($form, submitSuccessCallback){
	...
	var $this = this;
	if (!submitSuccessCallback) submitSuccessCallback = null;
	...
	this.form.find('.submit').on('click', function($form){    		$this.Submit(submitSuccessCallback);    	});
...
Form.prototype.Submit = function(submitSuccessCallback) {
	var $thisForm = this.form,
		$this = this;
	...
	this.Validate();
		...
		if ( proceed ) {
			...
			$.post('/ajax', data, function(response){
				...
				if (submitSuccessCallback) submitSuccessCallback();
