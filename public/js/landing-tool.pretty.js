...
;
(function($) {
  var applyAutoProceedLogicForToolForms, formOptions;
  formOptions = {
    validateOnTyping: true,
    preserveValuesAfterRefresh: false,
    customTransition: function($sectionToReveal, formInstance) {
      ...
      return setTimeout(function() {
        return $sectionToReveal.find('input[type="text"], input[type="email"], input[type="tel"]').first().focus();
      }, 275);
    },
    callbackOnPrepare: function(formInstance) {
      formInstance.form.find('.step').each(function() {
        var $step;
        $step = jQuery(this);
        return $step.addClass('show').removeClass('show hide');
      });
      return formInstance.form.find('.step').first().addClass('show');
    }
  };
  applyAutoProceedLogicForToolForms = function(form, formInstance) {
    return form.on('form_filled', util.debounce(function() {
      var $activeStep, hasButton;
      $activeStep = formInstance.step.current;
      hasButton = $activeStep.find('.next, .button, .submit').length !== 0;
      if (!$activeStep.hasClass('step_filled')) {
        return;
      }
      return setTimeout(function() {
        if (!hasButton) {
          return formInstance.Next();
        }
      }, 150);
    }, 120));
  };
  if ($('.form_tool-form').length) {
    return $('.form_tool-form').each(function() {
      var formInstance, getQuoteBlock;
      new Form($(this), formOptions);
      formInstance = $(this).data('Form');
      getQuoteBlock = formInstance.form;
      return applyAutoProceedLogicForToolForms(getQuoteBlock, formInstance);
    });
  }
})(jQuery);
