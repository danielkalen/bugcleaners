...
(function($) {
  var Notice, NoticeActions;
  if ($('.notices').length === 0) {
    $('body').prepend('<div class="notices"></div>');
  }
  this.notify = function(type, title, message, buttonText, altAction) {
    if (type == null) {
      type = 'ok';
    }
    if (title == null) {
  ...
  Notice = function(type, title, message, buttonText, altAction) {
  ...
  Notice.prototype.append = function() {
    return this.el.appendTo($('.notices').last());
  };
  ...
  Notice.prototype.reveal = function() {
    return setTimeout(function() {
      return $('.notices').last().addClass('reveal');
    }, 0);
  };
...
(function($) {
  ...
  Subnotice.prototype.attachEvents = function() {
    var subnoticeObject;
    subnoticeObject = this;
    return this.el.children('.subnotice-close').on('click', function() {
...
(function($) {
  var DB, PAGES, POSTTYPES, PageItem, Post, PostType, SIDEBAR, SidebarItem, Variation, alreadySetMinMax, appendDynamicBlocks, applicableLeads, fieldTemplates, generateNewSlug, initForm, initSortables, makeGroupSortable, sectionIDs, sectionOffsets, setMinMax, setSidebarPosition, sidebarMinMax, updateSectionOffsets;
  fieldTemplates = {};
  ...
  if (isPageManagement) {
    ...
    Variation.prototype.clone = function() {
      ...
      return subnotify('info', "Variation was successfuly cloned. Don't forget to save it in order to apply the changes to the database", 5000);
    };
    Variation.prototype.disable = function() {
      ...
      if (this.page.el.find('.manage-content-list-item').not('disabled').length > 1) {
        ...
        return notify('ok', "Can't disable variation", 'You must have at least one variation enabled per page');
      }
    };
    Variation.prototype["delete"] = function() {
      if (this.page.variations.length > 1) {
        return notify('yesno', "Delete Variation " + (this.index + 1), 'Are you sure you want to delete this variation? This cannot be undone.').then((function(_this) {
          return function() {
            ...
            return DB.variation.remove({
              ...
              'index': _this.index,
              'cb': function(res) {
                return subnotify('success', "Variation #" + (_this.index + 1) + " successfuly removed", 3000);
              }
            });
          };
        })(this));
      } else {
        return notify('ok', "Can't delete this variation", 'Each page must have at least one variation that\'s enabled');
      }
    };
  ...
  if (isPageManagement) {
    ...
    PageItem.prototype.disable = function() {
      this.enabled = !this.enabled;
      return notify('yesno', "Disabling " + this.name, "Are you sure you want to disable this page? It won't be accessible on the front end.").then((function(_this) {
        return function() {
          return DB.page.update({
            'id': _this.el.attr('id'),
            'name': 'enabled',
    ...
    PageItem.prototype.remove = function() {
      ...
      if (isLastPage) {
        return notify('ok', 'Why would you do that?', 'You cannot delete the last page in the database.');
      }
      return notify('yesno', "Deleting " + this.name, 'Are you sure you want to delete this page from the database? This cannot be undone.').then((function(_this) {
        return function() {
          ...
          if (_this.id) {
            return DB.page.remove({
              ...
              'cb': function(res) {
                if (res.success) {
                  subnotify('success', "Page " + _this.name + " was successfuly deleted.", 3000);
  ...
  if (isPostManagement) {
    ...
    Post.prototype["delete"] = function() {
      if (this.postType.posts.length > 1) {
        return notify('yesno', "Deleting " + this.name, "Are you sure you want to delete this " + this.single + " from the database? This cannot be undone.").then((function(_this) {
          return function() {
            ...
            if (_this.id) {
              return DB.post.remove({
                ...
                'cb': function(res) {
                  if (res.success) {
                    return subnotify('success', _this.name + " was successfuly removed.");
                  }
                }
              });
            }
        ...
        return notify('ok', 'How will you explain this to your users?', "You must have at least one " + this.single + " in the database.");
      }
    };
